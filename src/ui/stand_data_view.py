...
class StandDataView():
    ...
    def _initialize_stand_data(self):
        ...
        if self._plot_service.return_trees():
            ...
            mean_d = ttk.Label(master=self._frame, text=f"Keskiläpimitta (cm): {self._plot_service.mean_diameter():0.2f}")
            mean_h = ttk.Label(master=self._frame, text=f"Keskipituus (m): {self._plot_service.mean_height():0.2f}")
            sum_v = ttk.Label(master=self._frame, text=f"Tilavuus (m3/ha): {self._plot_service.sum_vol():0.3f}")
